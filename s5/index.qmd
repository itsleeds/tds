...
In this session, we will learn how to use routing engines and network analysis. The contents of the session are as follows:
...
# Review Homework (10 mins)

Your homework last week was to read Chapters 2–5 of Geocomputation with R, which introduced geographic data models, attribute and spatial data operations, and geometry operations. Bonus tasks were to explore the tmap and ggplot2 packages for spatial and data visualisation.

Divide into groups of 2–3 people and share your thoughts on the following:

* What is a spatial join, and when might you use one in a transport analysis context?
* What geometry operations did you find most relevant to transport data work? Can you think of a use case?
...
The Python packages we will be using for routing and network analysis:

```python
import geopandas as gpd       # Spatial vector data
import osmnx as ox            # Street network download and routing
import networkx as nx         # Graph and network algorithms
import matplotlib.pyplot as plt # Plotting and map visualization
import shapely.geometry as sg # Geometric operations
```

### R

If you have not installed the packages beforehand, you can use:
...
if (requireNamespace("lwgeom", quietly = TRUE)) library(lwgeom)
if (requireNamespace("stplanr", quietly = TRUE)) library(stplanr)
if (requireNamespace("dodgr", quietly = TRUE)) library(dodgr)
if (requireNamespace("opentripplanner", quietly = TRUE)) library(opentripplanner)
if (requireNamespace("osmextract", quietly = TRUE)) library(osmextract)
tmap_mode("plot")
```
...
![OTP screenshot](images/otp_screenshot.png)
...
ip = "otp.atfutures.org" # an actual server
otpcon = otp_connect(
  hostname = ip, 
  ssl = TRUE,
  port = 443,
...
```python
centroids = gpd.read_file("https://github.com/itsleeds/tds/releases/download/22/NTEM_cents.geojson")
...
2. Plot the `desire_lines` and `centroids` objects to show the number of travellers on each desire line and the locations of all centroids.
...
desire_lines.plot(column="all", cmap="viridis_r", linewidth=desire_lines["all"] / 1000, ax=ax)
centroids.plot(color="red", markersize=15, ax=ax)
ax.set_axis_off()
plt.title("Desire lines and centroids")
plt.show()
```
...
This dataset has desire lines, but most routing packages need start and endpoints, so we will extract the start and endpoints.

**Exercise**

4. Produce a data frame called `desire_top` which contains the top three `desire_lines` for all travellers.
...
```python
desire_top = desire_lines.nlargest(3, "all")
print(desire_top[["from", "to", "all"]])
```
...
5. We need to extract start and end points from those desire lines. We would also like to give each place an ID value:
...
fromPlace = gpd.GeoDataFrame(
    {"id": desire_top["from"]},
    geometry=desire_top.geometry.apply(lambda g: Point(g.coords[0])),
    crs=desire_lines.crs
)
toPlace = gpd.GeoDataFrame(
    {"id": desire_top["to"]},
    geometry=desire_top.geometry.apply(lambda g: Point(g.coords[-1])),
    crs=desire_lines.crs
)
```
...
Experiment with some different isochrones by changing the mode, and start location. Try overlaying the OD data on top of you isochrones. Can you see a relationship between travel time and travel demand?
...
if (file.exists("s4/isochrone.Rds")){isochrone = readRDS("s4/isochrone.Rds")}
...
* Redo exercise 11 but make sure you always select the fastest option. You may need to re-download the `routes_transit` data if you have overwritten the original data.

## Network Analysis (dodgr)

**Note** Some people have have problems running dodgr on Windows, if you do follow these [instructions](https://github.com/itsleeds/tds/blob/main/s4/dodgr-install.md).

We will now analyse the road network using network analysis tools. Network analysis can take a very long time on large areas, so we will use the example of the [Isle of Wight](https://en.wikipedia.org/wiki/Isle_of_Wight), which is ideal for transport studies as it is small, but has a full transport system.

::: {.panel-tabset}

### Python

In Python, we use **OSMnx** and **NetworkX** to download the network and compute network metrics like betweenness centrality:
...
# Download drivable road network for Isle of Wight
G = ox.graph_from_place("Isle of Wight, UK", network_type="drive")

# Calculate betweenness centrality (approximated with k=100 for speed)
bc = nx.betweenness_centrality(G, weight="length", k=100)
nx.set_node_attributes(G, bc, "centrality")

# Plot network colored by centrality
nc = ox.plot.get_node_colors_by_attr(G, "centrality", cmap="plasma")
fig, ax = ox.plot_graph(G, node_color=nc, node_size=15, edge_linewidth=0.5, show=False, close=False)
plt.title("Isle of Wight: Road Network Betweenness Centrality")
plt.show()
```

### R

In R, we use `osmextract` and `dodgr`:
...
# Build a graph
graph = dodgr::weight_streetnet(roads)
estimate_centrality_time(graph)
centrality = dodgr::dodgr_centrality(graph)
centrality_sf = dodgr::dodgr_to_sf(centrality)
```
...
14. Try contracting the graph before calculating centrality (`dodgr::dodgr_contract_graph` in R or simplifying the graph in Python). How does this affect computation time and results?

**Bonus Exercises**

* Work though the OpenTripPlanner vignettes [Getting Started](https://docs.ropensci.org/opentripplanner/articles/opentripplanner.html) and [Advanced Features](https://docs.ropensci.org/opentripplanner/articles/advanced_features.html) to run your own local trip planner for the Isle of Wight.
