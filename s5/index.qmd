...
bibliography: ../tds.bibtitle: "Session 5: AI for Transport Planning"title: "Session 5: Visualising transport data"

toc: true

execute: bibliography: ../tds.bibbibliography: ../tds.bib

  cache: true

  eval: truetoc: truetoc: true
...
Artificial Intelligence (AI) is rapidly transforming transport planning and data science.editor: editor: 
...
Artificial Intelligence (AI) is rapidly transforming transport planning and data science.In this session, we will build on the **routing techniques** from Session 4 by exploring **data visualization** methods for transport analysis.

-   **Complex spatial data** (networks, zones, flows)

-   **Large datasets** (GPS traces, survey data, traffic counts)In this session, we explore how AI tools—particularly **GitHub Copilot** and **large language models (LLMs)**—can enhance productivity, code quality, and problem-solving in transport data analysis.By the end of this session, you should be able to:
...
1.  **Code generation** - Writing boilerplate, data transformations, plotting-   Visualize OD lines and proportional symbol maps

2.  **Debugging** - Identifying errors and suggesting fixes

3.  **Documentation** - Generating comments, README files, and reports-   Understand the role of AI in modern transport planning workflows-   Compare walking, driving, and cycling flows

4.  **Learning** - Explaining unfamiliar functions or concepts

5.  **Optimization** - Suggesting more efficient algorithms or approaches-   Use GitHub Copilot effectively for R and Python coding-   Aggregate flows along the road network



# GitHub Copilot for Transport Data Science-   Apply prompt engineering techniques for transport data tasks-   Identify critical road segments via network centrality
...
GitHub Copilot is an **AI pair programmer** that suggests code completions in your editor (VS Code, RStudio, etc.).-   Integrate AI-assisted coding into reproducible research practices# Setup
...
# Why AI for Transport Planning?Below are the libraries we will use throughout this session:
...
2.  Install the Copilot extension in your IDE (VS Code, RStudio, etc.)Transport planning involves:```{r}

3.  Sign in with your GitHub account

4.  Start coding - Copilot will suggest completions automaticallyknitr::opts_chunk$set(



## Practical Examples-   **Complex spatial data** (networks, zones, flows)  message = FALSE,



### Example 1: Loading and Filtering Transport Data-   **Large datasets** (GPS traces, survey data, traffic counts)  warning = FALSE,



**Prompt (as a comment in R):**-   **Interdisciplinary methods** (statistics, optimization, visualization)  fig.width = 7,



```{r}-   **Reproducibility requirements** (code, documentation, version control)  fig.height = 5,

#| eval: false

# Load cycle hire data from London and filter trips longer than 30 minutes  out.width = "700px"
...
#| eval: false1.  **Code generation** - Writing boilerplate, data transformations, plotting# Load necessary libraries

library(dplyr)

library(readr)2.  **Debugging** - Identifying errors and suggesting fixeslibrary(opentripplanner)  # Routing engine (OpenTripPlanner client)



# Load cycle hire data3.  **Documentation** - Generating comments, README files, and reportslibrary(sf)               # Spatial data handling

cycle_data = read_csv("cycle_hire.csv")

4.  **Learning** - Explaining unfamiliar functions or conceptslibrary(tmap)             # Thematic mapping

# Filter trips longer than 30 minutes

long_trips = cycle_data |>5.  **Optimization** - Suggesting more efficient algorithms or approacheslibrary(stplanr)          # Transport data functions
...
### Example 2: Creating an Interactive Map# GitHub Copilot for Transport Data Sciencelibrary(osmextract)       # OSM data handling



**Prompt:**library(dodgr)            # Network analysis



```{r}## What is GitHub Copilot?```
...
GitHub Copilot is an **AI pair programmer** that suggests code completions in your editor (VS Code, RStudio, etc.).```{r}
...
tm_shape(stations) +
  ...
  tm_dots(size = 0.5, col = "blue", alpha = 0.7) +1.  Verify your student status at [education.github.com](https://education.github.com)Flow maps are useful for understanding the volume of travel between origins and destinations.

  tm_basemap("OpenStreetMap")

```2.  Install the Copilot extension in your IDE (VS Code, RStudio, etc.)In this section, we will:
...
**Prompt:**4.  Start coding - Copilot will suggest completions automatically1.  Load desire lines (flows) data from a GeoJSON file.



```{r}2.  Visualize these lines with widths or colors proportional to demand.

#| eval: false

# Calculate cycling routes between origin-destination pairs using stplanr## Practical Examples3.  Optionally aggregate route geometries for more realistic depiction of flows along an actual road network.
...
### Example 1: Loading and Filtering Transport Data```{r}
...
library(sf)**Prompt (as a comment in R):**desire_lines = read_sf("https://github.com/ITSLeeds/TDS/releases/download/22/NTEM_flow.geojson") |>



# Create OD data  select(from, to, all, walk, drive, cycle)
...
od = data.frame(
  ...
  to_lon = c(-1.54, -1.53),#| eval: falsedim(desire_lines)

  to_lat = c(53.79, 53.78)

)# Load cycle hire data from London and filter trips longer than 30 minutes# Let's take the top 50 car trips for demonstration



# Convert to spatial lines```desire_lines_top = desire_lines |>
...
routes = route_cyclestreets(od_sf, plan = "fastest")**Copilot might suggest:**  head(50)
...
```{r}# Quick map to see the distribution of car trips

### ✅ Do:

#| eval: falsetm_shape(desire_lines_top) +
...
-   **Use Copilot for learning** - examine suggestions to understand new approacheslibrary(readr)    lwd = "drive",
...
-   **Share sensitive data** in prompts (API keys, personal information)cycle_data = read_csv("cycle_hire.csv")  tm_layout(legend.bg.color = "white")
...
long_trips = cycle_data |># Proportional Symbol Flow Maps
...
4.  **Example** - Show desired input/output if possible### Example 2: Creating an Interactive Map
...
### Poor Prompt:**Prompt:**# Summarize total flows by origin
...
# Make a map```{r}origin_flows = desire_lines |>
