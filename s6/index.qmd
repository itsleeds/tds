...
In this session, we will explore techniques for joining, combining and aggregating datasets in Python and R, particularly for spatial data. We'll work with road crash data (STATS19), Lower Super Output Area (LSOA) boundaries, and census population data for West Yorkshire. Through this session, you'll learn:
...
4. How to create professional-looking reports with Quarto
...
slides for this session: [link](https://docs.google.com/presentation/d/11LmlCF2TRHrAOgLJ8QhXdXnxNBhbUY6K/edit?usp=sharing&ouid=113747908446540977697&rtpof=true&sd=true)
...
```python
import geopandas as gpd       # Simple features and spatial data operations
import pandas as pd           # Tabular data manipulation and aggregation
import matplotlib.pyplot as plt # Thematic map plotting
```
...
The full 2019-2023 download is time-consuming and can slow rendering significantly.
For routine rendering and preview, this page now downloads one year only (2023).

If you need the full historical collision dataset, you can retrieve all records from 1979 onward by setting `year = 1979` in `get_stats19()`.
:::

```{r}
#| warning: false
#| message: false
# Download STATS19 crash data (fast preview mode: single year)
if (!curl::has_internet()) {
  assignInNamespace("has_internet", function(...) TRUE, ns = "curl")
}
crashes = get_stats19(year = 2023, type = "accidents", ask = FALSE)
```
...
For fast rendering, we use one year of data by default. If needed, you can switch back to a multi-year download.

```{r}
# Check the dimensions of the dataset used in this run
dim(crashes)
```

`bind_rows()` from dplyr appends rows from multiple datasets into one dataset.
...
lsoa_wy = gpd.read_file("https://github.com/itsleeds/tds/releases/download/2025/p6-lsoa_boundary_wy.geojson")
lsoa_wy = lsoa_wy[["lsoa21cd", "lsoa21nm", "geometry"]]
print(f"Number of LSOAs in West Yorkshire: {len(lsoa_wy)}")
```
...
```python
# Spatial join: match each crash point to containing LSOA polygon
crashes_in_lsoa = gpd.sjoin(crashes_wy, lsoa_wy, how="left", predicate="within")
print("Matched crashes:", len(crashes_in_lsoa))
```
...
crashes_in_lsoa = st_join(crashes_wy, lsoa_wy)
...
```python
# Aggregate crash counts by LSOA
lsoa_crashes_count = (
    crashes_in_lsoa.groupby("lsoa21cd")
    .agg(
        fatal_crashes_n=("collision_severity", lambda s: (s == "Fatal").sum()),
        serious_crashes_n=("collision_severity", lambda s: (s == "Serious").sum()),
        slight_crashes_n=("collision_severity", lambda s: (s == "Slight").sum()),
        all_crashes_n=("collision_severity", "count"),
    )
    .reset_index()
)
print(lsoa_crashes_count.head())
```
...
lsoa_crashes_count = crashes_in_lsoa |>
  ...
  group_by(lsoa21cd) |>
  summarise(
    fatal_crashes_n = sum(collision_severity == "Fatal"),
    serious_crashes_n = sum(collision_severity == "Serious"),
    slight_crashes_n = sum(collision_severity == "Slight"),
...
| Join Type      | `dplyr` (R)                        | `pandas` (Python)                       | Description |
|----------------|------------------------------------|-----------------------------------------|-------------|
| **Inner Join** | `inner_join(df1, df2, by = "key")` | `df1.merge(df2, on="key", how="inner")` | Keeps only matching rows in both datasets. |
| **Left Join**  | `left_join(df1, df2, by = "key")`  | `df1.merge(df2, on="key", how="left")`  | Keeps all rows from `df1`, adds matching rows from `df2`. |
| **Right Join** | `right_join(df1, df2, by = "key")` | `df1.merge(df2, on="key", how="right")` | Keeps all rows from `df2`, adds matching rows from `df1`. |
| **Full Join**  | `full_join(df1, df2, by = "key")`  | `df1.merge(df2, on="key", how="outer")` | Keeps all rows from both datasets. |
| **Semi Join**  | `semi_join(df1, df2, by = "key")`  | `df1[df1["key"].isin(df2["key"])]`      | Keeps rows from `df1` that have matches in `df2`. |
| **Anti Join**  | `anti_join(df1, df2, by = "key")`  | `df1[~df1["key"].isin(df2["key"])]`     | Keeps rows from `df1` that do **not** have matches in `df2`. |
:::
...
```python
# Left join crash statistics onto spatial boundaries
lsoa_crashes_wy = lsoa_wy.merge(lsoa_crashes_count, on="lsoa21cd", how="left")
count_cols = ["all_crashes_n", "fatal_crashes_n", "serious_crashes_n", "slight_crashes_n"]
lsoa_crashes_wy[count_cols] = lsoa_crashes_wy[count_cols].fillna(0)

zero_crash_lsoas = (lsoa_crashes_wy["all_crashes_n"] == 0).sum()
print(f"LSOAs with zero recorded crashes: {zero_crash_lsoas}")
```
...
2. Add it to your YAML header:
   ...
   ---
   ```
3. Citations will automatically appear in your document with a References section at the end
...
1. **Title and metadata** (author, date, affiliation)
2. **Abstract or Executive Summary** (brief overview)
3. **Introduction** (context and objectives)
4. **Methods** (data sources, analysis techniques)
...
8. **References** (automatically generated by Quarto)
...
- [Quarto citations guide](https://quarto.org/docs/authoring/footnotes-and-citations.html)
- [Quarto cross-references](https://quarto.org/docs/authoring/cross-references.html)
- [R Markdown: The Definitive Guide](https://bookdown.org/yihui/rmarkdown/)
- [Building Reproducible Analytical Pipelines with R](https://raps-with-r.dev/) [@rodrigues_building_2023]
