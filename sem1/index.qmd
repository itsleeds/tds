---
title: "Seminar 1: Processing and visualising traffic datasets"
...
if (!requireNamespace("osmextract", quietly = TRUE)) {
  install.packages("osmextract", repos = "https://p3m.dev/cran/__linux__/jammy/latest")
}
library(sf)
library(tidyverse)
library(tmap)
...
We will start by loading the location of the BRT stations from a checked-in GeoJSON snapshot of the public ArcGIS dataset. Using a local copy keeps the seminar reproducible even when the upstream service is temporarily unavailable.
...
tm_stations_sf = gpd.read_file("Estaciones_Troncales_de_TRANSMILENIO.geojson")
tm_stations_sf["num_est"] = tm_stations_sf["numero_estacion"]
print(tm_stations_sf[["num_est", "nombre_estacion", "geometry"]].head())
```
...
tm_stations_sf <- read_sf("Estaciones_Troncales_de_TRANSMILENIO.geojson")
tm_stations_sf$num_est <- tm_stations_sf$numero_estacion
```
...
Let's quickly visualise the location of the stations. As we imported the data as a spatial object, we can easily visualise it using `matplotlib` in Python, or `tmap` / `ggplot2` in R.
...
tm_stations_sf.plot(ax=ax, color="darkblue", markersize=20)
ax.set_title("Bogotá BRT Stations (TransMilenio)")
plt.show()
```
