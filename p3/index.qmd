...
In this section, we will be working with origin-destination data in London Cycle Hire System (LCHS). As a case study, we are intersted in understanding how a strike in London Underground might impact the origin-destinaiton pairs in the LCHS. In this section, we all also be dealing with origin-destination data with temporal variable.

TfL provide various open data on cycling (link: https://cycling.data.tfl.gov.uk), to make it easier, we have cleaned the data that will be used in this session, and they can be downloaded from (link: xxxx link to the dataset)
...
bike_docking_stations = st_read("data/bike_docking_stations.geojson")
bike_trips = read.csv("data/bike_trips.csv")
...
This is a cleaned data, and we have only included three days' data, including the tube strike day and a day seven days before and a day seven days later for comparison. for example, we can check the unique values in the days. To fulfill this, we would like the time in a better format that is easier to deal with, hence we will firstly make them in POSIXct format.

What is Poxits format
...
We are very intersted in understanding if the increase in trips are spatially homoginised, or there are specific stations has generated and/or attracted lots of trips. Let make a series of maps to visualise them
...
od_non_strike <- bike_trips %>%
  ...
  group_by(start_station_id, end_station_id) %>%
  summarise(count_non_strike = n()/2)


od_change = od_strike %>% full_join(od_non_strike) %>%
...
od_change = filter(od_change, start_station_id != end_station_id) %>%
  ...
  mutate(ratio_change = count_strike /(count_strike - count_non_strike))



od_inter_strike = filter(od_strike, start_station_id != end_station_id)
strike_desire_lines = od2line(od_inter_strike,bike_docking_stations
                       )
...
How tube strike impacts the availability of available bike and docks?
```{r}
