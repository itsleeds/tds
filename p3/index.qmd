...
The spatial distribution of change may also be uneven. One assumption is that bike docking stations closer to Tube stations experience higher travel activity than those farther away. To quickly assess whether there is a difference, we can create buffers around Tube stations and calculate the changes in bike docking stations within these buffers. We can then compare these changes to those in docking stations outside the buffers.

Now, let create the buffer zones for tube stations in central london
...
# visualise the buffers, tube stations and bike docking stations
ggplot()+
  geom_sf(data = tube_stations_central_london_buffer, fill ="#3ba2c7", color = NA, alpha =0.3)+
...
```{r}
bike_trips |> group_by(start_station_id, date, type_day) |>
  summarise(count = n()) |>
  left_join(bike_docking_stations |> st_drop_geometry(), by = c("start_station_id" = "station_id")) |>
  group_by(inside_tube_buffer,date) |>
  summarise(mean_count = mean(count)) |>
  ggplot()+
  geom_bar(aes(x= as.factor(date), y = mean_count, fill = inside_tube_buffer), stat = "identity",position = position_dodge2())
```

Whist above temporal and spatial analysis provide useful summaries, visual analysis of the origin-destination changes allow us to characterise with greater richness the nature of changes in response to the strike events. Let' try to calculate the changes in o-d and map them!
...
ggplot() +
  geom_sf(
    data = change_desire_lines |> filter(count_change >= 1) |> arrange(count_change), # Define the data frame used to make the map
    aes(
