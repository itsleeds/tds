...
1.  Print the `vehicle_type` column of `crashes`.

<details>
<summary>Solution (Python & R)</summary>
...
In R the `$` symbol is used to refer to elements of a list/dataframe:
...
2.  Subset the crashes with index slicing syntax.
...
```python
crashes.iloc[0, :]  # first row
crashes.iloc[:, 0]  # first column
crashes.iloc[0, 0]  # first row, first column
```
...
3.  Subset a specific column or element.
...
4.  What is the data type/class of the objects created by each of the previous exercises?
...
This example shows how to solve a problem originally designed to teach Excel from the command line, in Python and R.
...
tms = pd.read_csv("transport-software.csv").iloc[:5]
tms = tms.sort_values("Citations", ascending=False)
print(tms)
```
...
Challenge: reproduce the code in Python using the `pystats19` package:
