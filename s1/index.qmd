...
An important component of data science is visualising datasets. This is especially critical when dealing with large volumes of data, as visualisations help convey complex information in an easily interpretable format. Consider the histogram plots of average trip lengths over a week in the UK.
...
As a data scientist, you often need to explore datasets in depth to uncover underlying patterns or differences. The direction of this investigation is largely guided by the research question.
...
Don't they largely look same? Can you stop here and infer that trip length distribution for weekdays and weekend are largely similar? You might depending on the resources at your disposal but from an academic point of view we need to think what are other potential dimensions where they could be different.
...
ggplot(NTS_data, aes(x = sd_Total_Distance_wk)) +
  geom_histogram(binwidth = 0.5, fill = "darkblue") + #,colour="white"
  labs(
...
ggplot(NTS_data, aes(x = sd_Total_Distance_wknd)) +
  geom_histogram(binwidth = 0.5, fill = "darkred") + #,colour="white"
  labs(
