...
```python
import pandas as pd
casualty_type = ["pedestrian", "cyclist", "cat"]
casualty_age = [20, 40, 60]
crashes = pd.DataFrame({"casualty_type": casualty_type, "casualty_age": casualty_age})
crashes["vehicle_type"] = ["car", "bus", "tank"]
print(crashes["vehicle_type"])
print(crashes.iloc[0, :])
print(crashes.iloc[:, 0])
print(crashes.iloc[0, 0])
```
...
try:
  ...
  print("Skipping polars example: install polars to run this chunk.")
else:
  casualty_type = ["pedestrian", "cyclist", "cat"]
  casualty_age = [20, 40, 60]
  crashes = pl.DataFrame({"casualty_type": casualty_type, "casualty_age": casualty_age})
  crashes = crashes.with_columns(vehicle_type = pl.Series(["car", "bus", "tank"]))
  print(crashes["vehicle_type"])
  print(crashes[0, :])
  print(crashes[:, 0])
  print(crashes[0, 0])
...
## Practical example: Excel from the command line

This example shows how to solve a problem originally designed to teach Excel from the command line, in R and Python.
The task is to load a dataset of transport modelling software, take the top 5 rows, and sort them by Citations in descending order.
...
try:
  ...
  tms = pl.read_csv("transport-software.csv")[:5]
  # Note: polars has a read_excel function, e.g. pl.read_excel("data.xlsx")
  tms = tms.sort("Citations", descending=True)
  print(tms)
```
...
import pystats19
# Get stats19 data for 2020
collisions = pystats19.get_stats19(year=2020, type='collision')
casualties = pystats19.get_stats19(year=2020, type='casualty')
vehicles = pystats19.get_stats19(year=2020, type='vehicle')

# Calculate percentage of population hurt
print(f"Percentage of population hurt: {(len(casualties) / 67e6) * 100}%")
```
:::
