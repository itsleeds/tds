...
As per your [timetable](https://itsleeds.github.io/tds/schedule.html), the first session details are **[TBC]**.

**Location:** [TBC]

## Homework for next week (deadline: [TBC])

1. Ensure that you have the timetable stored safely in your calendar, so you do not miss important sessions or seminars.
...
Test that you have the necessary software installed by running the code below. We recommend using Python as the primary language, with R also supported:
...
# Download street network features around the University of Leeds
its = ox.features_from_address("University of Leeds, UK", tags={"highway": True}, dist=500)

# Create and save map
fig, ax = plt.subplots(figsize=(6, 4))
its.plot(column="highway", ax=ax, legend=False)
ax.set_axis_off()
plt.tight_layout()
