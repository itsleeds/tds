---
title: "Session 4: Origin-destination data"
toc: true
execute: 
  cache: true
  eval: true
...
In this session, we will learn how to use origin-destination data. To do that we need to learn a bit about geographic data first. The contents of the session are as follows:
...
# Getting started with GIS in Python and R
...
if (!requireNamespace("ggspatial", quietly = TRUE)) {
  install.packages("ggspatial", repos = "https://p3m.dev/cran/__linux__/jammy/latest")
}
...
1. Create a project folder with an appropriate name for this session (e.g. `practical3`)
...
tm_shape(desire_lines) + # Define the data frame used to make the map
  ...
  (if (exists("tm_scalebar")) tm_scalebar() else tm_scale_bar()) # Add a scale bar to the map
```
