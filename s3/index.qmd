...
In this session, we will learn how to use geogarphic and origin-destination data. The contents of the session are as follows:
...
Note that this session takes sections from Chapters 2 - 8 of [Geocomputation with R](https://r.geocompx.org). You should expand your knowledge by reading these chapters in full.
...
1. Create a project folder with an appropriate name for this session (e.g. `session3`)
...
This section is based on [Chapter 13 of Geocomputation with R](https://r.geocompx.org/transport.html). You should read this chapter in full in your own time.
...
2. Using knowledge about datasets and new skills covered in sessions 2 and 3, think of a simple research question that could be answered by analysing/visualising/modelling one of the datasets imported into R in your previous homework (or another dataset you have imported into R).
...
5. Paste the resulting code into a code chunk in a new .qmd file, e.g. called `gen-ai-test.qmd`.
    ...
    - Note: in the next session we will review the AI-generated code you will share the code with a colleague and they will try to run it.
