...
cycle_data = pd.read_csv("cycle_hire.csv")

# Filter trips longer than 30 minutes
long_trips = cycle_data[cycle_data["duration_minutes"] > 30]
```
...
```python
# Create an interactive map showing bike share stations in Leeds using geopandas
```
...
stations = gpd.read_file("bike_stations.geojson")

# Create interactive map
m = stations.explore(color="blue", tiles="OpenStreetMap")
m.save("map.html")
```
...
```python
# Calculate cycling routes between origin-destination pairs using OSMnx
```
...
# Create network and find shortest path
orig = (53.80, -1.55)
dest = (53.79, -1.54)
G = ox.graph_from_point(orig, dist=2000, network_type="bike")
...
3. Reflect on how AI tools could benefit or negatively impact transport planning
